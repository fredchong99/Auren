...
export const STR = {
  en: {
    ...
    ui: {
      ...
      observerNote: "Observer mode for your level — I watch silently and give the full debrief at the end. Toggle Feedback if you want live coaching.",
      nextSessionTitle: "Booked for your next session",
      returnTomorrow: "Return tomorrow ↻ · simulated",
      day2Sys: "Simulated · the next day · AUREN remembers",
      retestTitle: "Retest · new disguise, same trap",
      retestPass: "Pattern recognized — the behaviour transferred to a brand-new variation, unprompted.",
      retestFail: "Still forming — the same lever moved you in a new disguise. This pattern is rebooked.",
      levelLabel: "level",
    },
    bands: { novice: "Novice", developing: "Developing", confident: "Confident", advanced: "Advanced" },
    bandLine: (b) => `That places you at ${b} level. Your scenario difficulty — and how much I step in — is set from this, not from a menu.`,
    nextSessionBody: (sig) => `I will retest “${sig}” unprompted, in a new disguise, next session. Readiness is proven on the retest, not the first pass.`,
    day2Greet: (n, q) => `Welcome back, ${n}. Last session you said “${q}”. Today I stay silent — new situation, same trap underneath. Show me it stuck.`,
    day2Close: (n, pass) => pass
      ? `That, ${n}, is the difference between knowing and readiness — you did it without me. Your score has moved.`
      : `Same lever, new disguise, ${n} — and it worked again. That is exactly why we drill. I've rebooked this pattern.`,
    marcusPro: [
      "Good evening. Adam Lee passed me your contact — I run a private allocation desk. 40% net this quarter, independently audited. I onboard five clients a month, and I have one seat left.",
      "Sensible to ask. I'll send the performance sheet and two client references tonight. Adam has already allocated. The tranche closes Friday — shall I reserve your seat while you review?",
      "To hold the seat I need the initial commitment tonight — RM2,000 via the secure portal. After Friday the minimum triples. I'll send the link now, yes?",
    ],
    marcusDay2: "Me again 😊 Different thing — forget the group. A regulated gold desk, my own money is in. Deposit tonight, the position doubles by Monday. Adam said you'd be quick enough. In?",
    verdict: {
  ...
  ms: {
    ...
    ui: {
      ...
      observerNote: "Mod pemerhati untuk tahap anda — saya perhati secara senyap dan beri penilaian penuh di akhir. Tekan Maklum balas jika mahu bimbingan langsung.",
      nextSessionTitle: "Ditempah untuk sesi seterusnya",
      returnTomorrow: "Kembali esok ↻ · simulasi",
      day2Sys: "Simulasi · hari berikutnya · AUREN ingat",
      retestTitle: "Ujian semula · samaran baru, perangkap sama",
      retestPass: "Corak dikenal pasti — tingkah laku itu berpindah ke variasi yang serba baru, tanpa bantuan.",
      retestFail: "Masih terbentuk — tuil yang sama menggerakkan anda dalam samaran baru. Corak ini ditempah semula.",
      levelLabel: "tahap",
    },
    bands: { novice: "Baru", developing: "Berkembang", confident: "Yakin", advanced: "Mahir" },
    bandLine: (b) => `Ini meletakkan anda pada tahap ${b}. Kesukaran senario — dan sejauh mana saya campur tangan — ditetapkan daripada tahap ini, bukan daripada menu.`,
    nextSessionBody: (sig) => `Saya akan menguji semula “${sig}” tanpa amaran, dalam samaran baru, pada sesi seterusnya. Kesediaan dibuktikan pada ujian semula, bukan percubaan pertama.`,
    day2Greet: (n, q) => `Selamat kembali, ${n}. Sesi lepas anda berkata “${q}”. Hari ini saya diam — situasi baru, perangkap yang sama di sebaliknya. Tunjukkan ia kekal.`,
    day2Close: (n, pass) => pass
      ? `Itulah, ${n}, beza antara tahu dan bersedia — anda melakukannya tanpa saya. Skor anda meningkat.`
      : `Tuil yang sama, samaran baru, ${n} — dan ia berjaya lagi. Inilah sebabnya kita berlatih. Corak ini ditempah semula.`,
    marcusPro: [
      "Selamat malam. Adam Lee beri saya nombor awak — saya urus meja peruntukan persendirian. 40% bersih suku ini, diaudit bebas. Saya ambil lima klien sebulan, dan tinggal satu tempat.",
      "Wajar bertanya. Saya hantar helaian prestasi dan dua rujukan klien malam ini. Adam sudah melabur. Tranche tutup Jumaat — saya simpan tempat awak sementara awak semak?",
      "Untuk simpan tempat, komitmen awal perlu malam ini — RM2,000 melalui portal selamat. Selepas Jumaat minimum naik tiga kali ganda. Saya hantar link sekarang, ya?",
    ],
    marcusDay2: "Saya lagi 😊 Benda lain — lupakan kumpulan itu. Meja emas berlesen, duit saya sendiri pun ada. Deposit malam ini, kedudukan berganda menjelang Isnin. Adam kata awak cukup pantas. Nak?",
    verdict: {
  ...
  es: {
    ...
    ui: {
      ...
      observerNote: "Modo observador para tu nivel — observo en silencio y doy el análisis completo al final. Activa Feedback si quieres coaching en vivo.",
      nextSessionTitle: "Reservado para tu próxima sesión",
      returnTomorrow: "Volver mañana ↻ · simulado",
      day2Sys: "Simulado · al día siguiente · AUREN recuerda",
      retestTitle: "Repetición · nuevo disfraz, misma trampa",
      retestPass: "Patrón reconocido — la conducta se transfirió a una variación totalmente nueva, sin ayuda.",
      retestFail: "Aún en formación — la misma palanca te movió con un disfraz nuevo. Este patrón queda reprogramado.",
      levelLabel: "nivel",
    },
    bands: { novice: "Inicial", developing: "En desarrollo", confident: "Competente", advanced: "Avanzado" },
    bandLine: (b) => `Eso te sitúa en el nivel ${b}. La dificultad de tus escenarios — y cuánto intervengo — se fija a partir de aquí, no desde un menú.`,
    nextSessionBody: (sig) => `Volveré a poner a prueba “${sig}” sin avisar, con un nuevo disfraz, en la próxima sesión. La preparación se demuestra en la repetición, no en el primer intento.`,
    day2Greet: (n, q) => `Bienvenido de nuevo, ${n}. En la última sesión dijiste “${q}”. Hoy me quedo en silencio — situación nueva, la misma trampa debajo. Demuéstrame que se quedó.`,
    day2Close: (n, pass) => pass
      ? `Eso, ${n}, es la diferencia entre saber y estar preparado — lo hiciste sin mí. Tu puntuación ha subido.`
      : `La misma palanca con un disfraz nuevo, ${n} — y volvió a funcionar. Exactamente por eso entrenamos. Este patrón queda reprogramado.`,
    marcusPro: [
      "Buenas noches. Adam Lee me pasó su contacto — gestiono una mesa de asignación privada. 40% neto este trimestre, auditado de forma independiente. Acepto cinco clientes al mes y queda una plaza.",
      "Es sensato preguntar. Esta noche le envío la hoja de resultados y dos referencias de clientes. Adam ya ha invertido. El tramo se cierra el viernes — ¿le reservo la plaza mientras lo revisa?",
      "Para mantener la plaza necesito el compromiso inicial esta noche — 2.000 € por el portal seguro. Después del viernes el mínimo se triplica. ¿Le envío el enlace ahora?",
    ],
    marcusDay2: "Otra vez yo 😊 Algo distinto — olvida el grupo. Una mesa de oro regulada, mi propio dinero está dentro. Deposita esta noche y la posición se duplica el lunes. Adam dijo que eras rápido. ¿Entras?",
    verdict: {
