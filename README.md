...
3. **Readiness profile reveal — the AUREN Investor Readiness Score (AIRS) + level band** — she presents the five-dimension score with an assigned level (Novice / Developing / Confident / Advanced) generated from the conversation. The band routes the rest of the session: it selects the scam persona's sophistication (classic WhatsApp recruiter vs polished "allocation desk") and AUREN's coaching intensity (live intervention → silent observer with end-of-session debrief).
...
5. **Evidential scorecard + regulator glimpse** — she hands you your own typed words, turn-stamped with score deltas and a Certified / Needs review / Retrain verdict, plus the one anonymized data point a regulator receives (Cross-Jurisdictional value in 30 seconds). The scorecard closes with a **booked next session** — the one behaviour she will retest.
6. **The feedback loop — simulated Day-2 return** — "Return tomorrow ↻" jumps to the next day: she greets you by name, quotes your weakest moment from the last session, and retests the same behavioural pattern in a new disguise (a different scam, same trap). Pass and the Retest card reads "Pattern recognized"; fail and the pattern is rebooked. The continuity relationship, demonstrated live.
